        ...
        for (int i = 0, size = castlingState.length(); i < size; i++) {
            ...
            if (!FIELD_EMPTY.equals(state)) {
                ...
                case WHITE_KING_SYMBOL:
                    setupCastlingState(chessboard.getSquare(Squares.SQ_E, Squares.SQ_1),
                ...
                case BLACK_KING_SYMBOL:
                    setupCastlingState(chessboard.getSquare(Squares.SQ_E, Squares.SQ_8),
        ...
        for (String row : piecesStateString.split(ROW_SEPARATOR)) {
            ...
            for (int i = 0; i < row.length(); i++) {
                ...
                try {
                    ...
                    Piece piece = PieceFactory.getPieceFromFenNotation(game.getChessboard(), currChar, whitePlayer,
                            blackPlayer);
        ...
        for (int y = Chessboard.FIRST_SQUARE; y <= Chessboard.LAST_SQUARE; y++) {
            ...
            for (int x = Chessboard.FIRST_SQUARE; x <= Chessboard.LAST_SQUARE; x++) {
                ...
                if (null == piece) {
                    ...
                    result += piece.getPlayer().getColor() == Colors.WHITE ? symbol.toUpperCase()
                            : symbol.toLowerCase();
                }
            }
            if (0 != emptySquares) {
